...
function onlyLetters(stringNoN) {
  return stringNoN.replace(/[0-9]/g, "");
}
onlyLetters("Ho 24 anni ");
...
function whatDayIsIt() {
  ...
  let now = date.toLocaleDateString("it-IT", options);
...
/* ESERCIZIO 8
  ...
  Deve invocare la precedente funzione dice() il numero di volte specificato nel parametro, e 
  deve tornare un oggetto contenente una proprietà "sum":
  il suo valore deve rappresentare il totale di tutti i valori estratti con le invocazioni di dice().
  L'oggetto ritornato deve anche contenere una proprietà "values", contenente un array con tutti i valori estratti
...
console.log(howManyDays(6/2/2024))
/* ESERCIZIO 10
  Scrivi una funzione chiamata "isTodayMyBirthday" che deve ritornare true se oggi è il tuo compleanno, falso negli altri casi.
*/

function isTodayMyBirthday(compl) {
...
console.log(deleteProp("skills", object));

/* ESERCIZIO 12
  Scrivi una funzione chiamata "newestMovie" che trova il film più recente nell'array "movies" fornito.
*/

let newestMovie = (movie) => {
  let max = { Year: 1900 };
  movie.forEach((curr) => {
    let anno = parseInt(curr.Year);
    if (anno > max.Year) {
      max = curr;
    }
  })
...
const sumAllTheYears = (arr) => {
  ...
  return arr.reduce((temp, curr) => temp + parseInt(curr.Year), 0)
...
let searchByTitle = (arr) => {

  return arr.filter((el)=> el.Title.includes('Avengers'))

}
searchByTitle(movies)

/* ESERCIZIO 18
  Scrivi una funzione chiamata "searchAndDivide" che riceve una stringa come parametro e ritorna un oggetto contenente
   due array: "match" e "unmatch".
  "match" deve includere tutti i film dell'array "movies" fornito che contengono la stringa fornita all'interno del
...
for(let i = 0;i<movies.length;i++){
  ...
  titoli.match.push(movies[i].Title)
}
else
{
  titoli.unmatch.push(movies[i].Title)
}
...
*/
 ...
 tabella.forEach(el => console.log(table.innerText))
...
link.forEach(el=> el.style.backgroundColor ='red')
}
/* ESERCIZIO 24
...
let liSTa= function(){
  ...
  let myList= document.querySelector('#myList ul')
...
let list=function lista(){

  let li = querySelector('#myList ul')
